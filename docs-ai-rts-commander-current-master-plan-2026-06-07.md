...
| 157 | Next | `Refresh PC controlled-demo investor route evidence after F77 audit fixes` | Refresh command evidence metadata so it explicitly consumes the F77 audit report and F78 fix report while preserving route, damage/ejection, public-safe proxy and landscape phone proof lines. | PC investor route evidence refresh |
