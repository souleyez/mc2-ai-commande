...
| 130 | Next | `Refresh PC controlled-demo investor route evidence after F50 audit fixes` | Refresh command evidence metadata so it explicitly consumes the F50 audit report and the F51 fix report while preserving the five-preset investor route package. | PC investor route evidence refresh |
