...
```text
F2 map authoring contract sealed -> F3 web ranking contract
```
...
- AI 副官是可选能力，不拖慢本地战斗；
- 公开替换包路线讲得清楚；
- Android G3 真机 smoke、横屏 G4 Touch UI pass、G5 Mobile performance budget、G6 iOS feasibility gate 和 F2 map authoring contract 已通过；下一步进入 F3 web ranking contract。
...
| 平台化仍停在契约层 | 奖励权威和地图包契约已写清楚，排行和创作者边界还要补 | F3-F4 排在 F2 之后 |
...
| 81 | Next | `Plan web ranking prototype` | 排行、战绩、地图页和公开资料规划 | docs check |
| 82 | Later | `Plan creator economy boundary` | 创作者分成、皮肤、自定义、链上边界 | docs check |
