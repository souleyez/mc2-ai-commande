...
```text
Mobile G4 Landscape Touch Passed -> G5 Mobile performance budget
```
...
- AI 副官是可选能力，不拖慢本地战斗；
- 公开替换包路线讲得清楚；
- Android G3 真机 smoke 和横屏 G4 Touch UI pass 已通过；下一步进入 G5 Mobile performance budget。
...
| 78 | Later | `Define mobile performance budget` | 记录 FPS、内存、包体、加载、热量/电量基线 | docs + device evidence |
