...
```text
Mobile G5 Performance Budget Passed -> G6 iOS feasibility gate
```
...
- AI 副官是可选能力，不拖慢本地战斗；
- 公开替换包路线讲得清楚；
- Android G3 真机 smoke、横屏 G4 Touch UI pass 和 G5 Mobile performance budget 已通过；下一步进入 G6 iOS feasibility gate。
...
| 79 | Later | `Document iOS feasibility gate` | 记录 macOS/Xcode/签名/Metal/真机要求 | docs check |
| 80 | Later | `Plan map authoring prototype` | 地图包、触发、奖励引用和验证器规划 | docs check |
