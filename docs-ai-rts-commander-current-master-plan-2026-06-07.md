...
| 124 | Next | `Refresh PC controlled-demo investor route evidence after polish fixes` | Regenerate the command/route evidence from existing sidecars after F45 so the ignored markdown/json package carries the new route summary and landscape-phone proof line. | PC investor route evidence refresh |
