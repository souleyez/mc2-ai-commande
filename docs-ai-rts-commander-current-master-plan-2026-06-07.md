...
| 150 | Next | `Implement post-F71 PC controlled-demo investor route evidence refresh audit fixes` | Make the F71 audit findings and follow-ups visible in plan, evidence and handoff surfaces before the next command-evidence metadata refresh, keeping no-Unity-launch evidence and landscape-only phone scope. | PC investor route evidence refresh audit fixes |
