...
| 75 | Done | `Add Android G3 when-ready runner` | 等待 adb `device` 后才调用真实 `android_device_smoke.ps1`；`-PlanOnly` 只预演，不安装、不启动 | `G3WhenReady: True` |
| 76 | Later | `Adapt command UI for mobile touch` | 状态行、Jet、地图、系统和 MechLab 在手机触控可用 | device smoke |
| 77 | Later | `Define mobile performance budget` | 记录 FPS、内存、包体、加载、热量/电量基线 | docs + device evidence |
| 78 | Later | `Document iOS feasibility gate` | 记录 macOS/Xcode/签名/Metal/真机要求 | docs check |
| 79 | Later | `Plan map authoring prototype` | 地图包、触发、奖励引用和验证器规划 | docs check |
| 80 | Later | `Plan web ranking prototype` | 排行、战绩、地图页和公开资料规划 | docs check |
| 81 | Later | `Plan creator economy boundary` | 创作者分成、皮肤、自定义、链上边界 | docs check |
...
62. Android G3 when-ready runner proves `run_android_g3_when_ready.ps1 -PlanOnly` reports `G3WhenReady: True` and `NoInstallOrLaunchUntilDeviceReady: True`, then only calls real `android_device_smoke.ps1` after the adb watch reports one authorized device.
