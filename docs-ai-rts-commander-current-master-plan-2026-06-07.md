...
| 151 | Next | `Refresh PC controlled-demo investor route evidence after F71 audit fixes` | Refresh command evidence metadata so it explicitly consumes the F71 audit report and F72 fix report while preserving route, damage/ejection, public-safe proxy and landscape phone proof lines. | PC investor route evidence refresh |
