...
```text
Mobile gate sealed through G6 -> F2 map authoring contract
```
...
- AI 副官是可选能力，不拖慢本地战斗；
- 公开替换包路线讲得清楚；
- Android G3 真机 smoke、横屏 G4 Touch UI pass、G5 Mobile performance budget 和 G6 iOS feasibility gate 已通过；下一步进入 F2 map authoring contract。
...
| 80 | Next | `Plan map authoring prototype` | 地图包、触发、奖励引用和验证器规划 | docs check |
| 81 | Later | `Plan web ranking prototype` | 排行、战绩、地图页和公开资料规划 | docs check |
