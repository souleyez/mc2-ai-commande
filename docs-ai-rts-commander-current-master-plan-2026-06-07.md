...
| 153 | Next | `Implement post-F74 PC controlled-demo investor route evidence refresh audit fixes` | Make the F74 audit findings and follow-ups visible in plan, evidence and handoff surfaces, keep new F-series artifact paths short, and prepare the next command-evidence metadata refresh to consume F74. | PC investor route evidence refresh audit fixes |
