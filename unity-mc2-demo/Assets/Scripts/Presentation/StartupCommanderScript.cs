...
namespace MC2Demo.Presentation
{
    public static class StartupCommanderScript
    {
        ...
        public static bool TryParseLine(string line, int lineNumber, out StartupCommanderScriptAction action, out string error)
        {
            ...
            if (string.Equals(verb, "inventory-bootstrap-smoke", StringComparison.OrdinalIgnoreCase))
            {
                if (!string.IsNullOrWhiteSpace(payload))
                {
                    error = "Inventory bootstrap smoke action does not accept arguments.";
                    return false;
                }

                action = StartupCommanderScriptAction.InventoryBootstrapSmoke(lineNumber, rawLine);
                return true;
            }
            ...
            if (string.Equals(verb, "assert-inventory-bootstrap-smoke", StringComparison.OrdinalIgnoreCase))
            {
                if (!string.IsNullOrWhiteSpace(payload))
                {
                    error = "Assert inventory bootstrap smoke action does not accept arguments.";
                    return false;
                }
    ...
    public sealed class StartupCommanderScriptAction
    {
        ...
        public static StartupCommanderScriptAction InventoryBootstrapSmoke(int lineNumber, string sourceLine)
        {
            return new StartupCommanderScriptAction
            {
        ...
        public static StartupCommanderScriptAction AssertInventoryBootstrapSmoke(int lineNumber, string sourceLine)
        {
            return new StartupCommanderScriptAction
            {
                Kind = StartupCommanderScriptActionKind.AssertInventoryBootstrapSmoke,
                LineNumber = lineNumber,
                SourceLine = sourceLine ?? string.Empty
            };
        }
