...
| F45 | Next | `Implement post-F44 PC controlled-demo investor evidence polish fixes` | Apply the F44 follow-ups: compact investor route summary, explicit damage-demo screenshot/sidecar links, visible landscape-phone proof line, and easier proxy-language parsing without launching Unity unless a fresh recapture is intentionally scheduled. | PC investor evidence polish fixes |
