...
| F55 | Next | `Refresh PC controlled-demo investor route evidence after F53 audit fixes` | Refresh command evidence metadata so it explicitly consumes the F53 audit report and F54 fix report while preserving the route, damage/ejection, public-safe proxy and horizontal-phone proof lines. | PC investor route evidence refresh |
