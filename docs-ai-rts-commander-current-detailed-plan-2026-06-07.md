...
| F46 | Next | `Refresh PC controlled-demo investor route evidence after polish fixes` | Regenerate the ignored command evidence markdown/json from existing sidecars so the F45 route summary, damage links, horizontal-phone proof line and proxy parsing markers are reflected in the current investor evidence package. | PC investor route evidence refresh |
