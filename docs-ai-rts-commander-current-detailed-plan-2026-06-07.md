...
4. Android 等待期间完成的 PC1-PC57 前置包仍保留为证据；正式下一步不再停在 G3/G4，而是推进 G5 性能预算。
...
| G5 | Next | `Define mobile performance budget` | FPS、内存、包体、加载、热量/电量基线 | docs + device evidence |
...
- 当前计划 gate、handoff 和移动指挥模型预检接受 PC1-PC57、`Pass Android G3 device smoke`、横屏 `G4 Touch UI pass`，并把正式下一步推进到 `G5 Mobile performance budget`。
