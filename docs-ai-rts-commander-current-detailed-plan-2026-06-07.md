...
| F43 | Next | `Refresh PC controlled-demo investor evidence package after fixes` | Refresh the PC controlled-demo investor evidence package after F42, using existing sidecars or a rebuilt Windows player when safe, so the executive summary, preset highlights, proxy identity markers, damage callout and fast gate are represented in the generated evidence. | `capture_pc_controlled_demo_command_evidence.ps1` |
