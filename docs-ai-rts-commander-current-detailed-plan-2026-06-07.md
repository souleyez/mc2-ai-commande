...
4. Android 等待期间完成的 PC1-PC57 前置包仍保留为证据；G5 性能预算已记录 Mi 11 Lite 基线，正式下一步推进 G6 iOS 可行性。
...
| G6 | Next | `Document iOS feasibility gate` | macOS/Xcode/签名/Metal/真机要求 | docs |
...
- 当前计划 gate、handoff 和移动指挥模型预检接受 PC1-PC57、`Pass Android G3 device smoke`、横屏 `G4 Touch UI pass` 和 `G5 Mobile performance budget`，并把正式下一步推进到 `G6 iOS feasibility gate`。
