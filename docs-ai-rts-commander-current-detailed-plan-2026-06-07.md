...
| F82 | Done | `Refresh PC controlled-demo investor route evidence after F80 audit fixes` | Refreshed command evidence metadata so it explicitly consumes the F80 audit report and F81 fix report while preserving route, damage/ejection, public-safe proxy and landscape phone proof lines. | `check_f82_pc_route_evidence_refresh.ps1` |
| F83 | Done | `Audit post-F82 PC controlled-demo investor route evidence refresh` | Audited the refreshed F82 command evidence package for F80/F81 source traceability, route readability, damage/ejection continuity, public-safe proxy wording, landscape phone proof visibility and Windows path-budget safety. | `audit_f83_pc_route_evidence_refresh.ps1` |
| F84 | Next | `Implement post-F83 PC controlled-demo investor route evidence refresh audit fixes` | Make F83 audit findings and follow-ups visible in plan, evidence and handoff surfaces, close the F85 consume-F83-audit-report contract, and keep the new F84 script/output path short. | PC investor route audit fixes |
...
- `check_current_plan_gate.ps1` 增加 Android G3 device status report gate；queue、handoff 和 mobile command preflight 同步到 PC1-PC83。
...
- `check_current_plan_queue.ps1`、`check_current_plan_gate.ps1`、`check_controlled_demo_handoff.ps1 -RunReadiness` 和 `check_mobile_command_model_preflight.ps1` 接受 PC1-PC83。
...
- `check_current_plan_gate.ps1` 增加 Android G3 when-ready plan gate；queue、handoff 和 mobile command preflight 同步到 PC1-PC83。
...
- 当前计划 gate、handoff 和移动指挥模型预检接受 PC1-PC83、`Pass Android G3 device smoke`、横屏 `G4 Touch UI pass`、`G5 Mobile performance budget`、`G6 iOS feasibility gate`、`F2 map authoring contract`、`F3 web ranking contract`、`F4 creator economy boundary`、`F5 server implementation boundary`、`F6 local main-server prototype`、`F7 document Unity main-server integration contract` 和 `F8 implement optional Unity main-server client adapter`，并把正式下一步推进到 `F9 wire optional Unity main-server adapter into launch/debrief smoke`。
...
- `check_current_plan_gate.ps1` 增加 Android ADB driver package gate；queue、handoff 和 mobile command preflight 同步到 PC1-PC83。
...
- 当前计划 gate、handoff 和移动指挥模型预检接受 PC1-PC83。
