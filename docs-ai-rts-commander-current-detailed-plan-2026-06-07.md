...
**Revision:** 2026-06-13 v144. This file is the fine-grained execution plan paired with `docs-ai-rts-commander-current-master-plan-2026-06-07.md`. H2 validator/build/smoke is green; G2 Android build smoke is green with a generated APK; PC1-PC84 and `F12 implement opt-in inventory-to-MechBay preview binding` through `F84 implement post-F83 PC controlled-demo investor route evidence refresh audit fixes` are complete; `F85 refresh PC controlled-demo investor route evidence after F83 audit fixes` is the formal next task. Mobile phones remain first-version landscape-only as 手机端横版 / horizontal phone build; portrait is not a first-slice support target.
...
4. Android 等待期间完成的 PC1-PC84 前置包仍保留为证据；F84 route evidence audit fixes 已完成，证据 gate: `scripts/unity/check_f84_pc_route_audit_fixes.ps1` -> `F84 PC route audit fixes check OK.`；正式下一步是 `F85 refresh PC controlled-demo investor route evidence after F83 audit fixes`。Mobile phones remain first-version landscape-only as the horizontal phone build; portrait is not a first-slice support target.
5. Platform/server integration markers remain closed: `F10 wire optional Unity inventory bootstrap smoke` and `F11 plan inventory-to-MechBay binding boundary`.
...
| M14 | PC/移动等待态优化 | Done for current pass | PC1-PC84 passed; the current pass now includes the F84 route evidence audit-fix closure and keeps the phone build landscape-only. |
...
| F84 | Done | `Implement post-F83 PC controlled-demo investor route evidence refresh audit fixes` | Made F83 audit findings and follow-ups visible in plan, evidence and handoff surfaces, closed the F85 consume-F83-audit-report contract, kept the new F84 script/output path short, and preserved the landscape-only phone contract. | `check_f84_pc_route_audit_fixes.ps1` |
| F85 | Next | `Refresh PC controlled-demo investor route evidence after F83 audit fixes` | Refresh command evidence metadata so it explicitly consumes the F83 audit report and F84 fix report while preserving route, damage/ejection, public-safe proxy and landscape phone proof lines. | PC investor route evidence refresh |
...
PC1-PC84 PC/移动等待态优化包已封口；手机端第一版固定横屏；F84 route evidence audit fixes 已完成，证据 gate: `scripts/unity/check_f84_pc_route_audit_fixes.ps1` -> `F84 PC route audit fixes check OK.`；正式下一步是 `F85 refresh PC controlled-demo investor route evidence after F83 audit fixes`。
