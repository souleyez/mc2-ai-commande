...
| F48 | Next | `Implement post-F47 PC controlled-demo investor route evidence audit fixes` | Implement the narrow F47 follow-ups so the route-audit findings and next fixes are visible in handoff and investor evidence docs without launching Unity or expanding mobile beyond horizontal phone scope. | PC investor route evidence audit fixes |
