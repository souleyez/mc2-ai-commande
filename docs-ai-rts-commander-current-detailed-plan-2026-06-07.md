...
| F50 | Next | `Audit post-F49 PC controlled-demo investor route evidence refresh` | Audit the refreshed F49 route evidence package for route-proof clarity, damage/ejection links, horizontal-phone proof visibility and public-safe proxy language before the next playable-demo polish step. | PC investor route evidence refresh audit |
