...
| F49 | Next | `Refresh PC controlled-demo investor route evidence after audit fixes` | Refresh the ignored route evidence package after F48 so command evidence/report metadata carries the route-audit fix closure and F49/F50 state without launching Unity unless source evidence requires it. | PC investor route evidence refresh |
