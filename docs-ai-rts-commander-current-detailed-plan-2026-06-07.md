...
| F33 | Next | `Implement PC controlled-demo command readability and formation fixes` | Apply F32 followups: widen default spacing, emit CommandReadability sidecar, capture live detached-order state, mark commander follow, and separate command/target cues. Keep first phone slice landscape-only. | PC command readability fixes |
