...
3. 当前移动端下一步是 `G4 Touch UI pass`，先让核心指挥模型在手机触控和竖屏/窄屏比例下可用。
4. Android 等待期间完成的 PC1-PC57 前置包仍保留为证据；正式下一步不再停在 G3，而是推进 G4。
...
| G4 | Next | `Adapt command UI for mobile touch` | 状态行、Jet、地图、系统和 MechLab 手机触控可用 | device smoke |
...
- 当前计划 gate、handoff 和移动指挥模型预检接受 PC1-PC57、`Pass Android G3 device smoke` 和 `G4 Touch UI pass`。
