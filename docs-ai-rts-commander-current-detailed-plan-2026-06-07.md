...
| F40 | Next | `Refresh PC controlled-demo investor-readiness evidence after fixes` | Rebuild/capture the PC controlled demo evidence after F39 so the spawn, hangar-contact, damage-demo, solo-order and solo-return reports include debrief summary and investor proxy visual sidecar fields. | PC investor-readiness evidence refresh |
