...
| F34 | Next | `Refresh PC controlled-demo command evidence after readability fixes` | Rebuild or reuse the current Windows player, recapture PC controlled-demo evidence including solo-order, and verify CommandReadability/CommanderFollow sidecars plus formation/contact spacing after F33. Keep first phone slice landscape-only. | PC command evidence refresh |
