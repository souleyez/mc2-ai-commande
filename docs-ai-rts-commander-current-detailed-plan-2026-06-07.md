...
| F36 | Next | `Implement post-F34 PC controlled-demo playable flow polish fixes` | Implement the F35 followups: contact pressure cue, damage/ejection-to-debrief clarity, solo return settled evidence, denser status rail, and unified playable-flow evidence while keeping mobile landscape-only. | post-F34 PC playable flow polish fixes |
