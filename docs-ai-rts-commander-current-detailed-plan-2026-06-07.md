...
| F39 | Next | `Implement post-F37 PC controlled-demo investor readiness fixes` | Implement the narrow F38 follow-ups: expose damage-demo debrief/repair consequence in the command evidence summary, improve high-visibility fallback model/proxy fidelity where safe, and add a concise investor demo route without expanding controls beyond the landscape mobile command model. | PC investor-readiness fixes |
