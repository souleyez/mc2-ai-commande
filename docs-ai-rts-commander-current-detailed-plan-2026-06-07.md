...
| F37 | Next | `Refresh PC controlled-demo playable-flow evidence after polish fixes` | Rebuild/refresh PC captures for spawn, hangar-contact, damage-demo, solo-order and solo-return so one report links command clarity, contact pressure, damage story, sparse HUD and landscape-only mobile contract after F36. | PC playable-flow evidence refresh |
