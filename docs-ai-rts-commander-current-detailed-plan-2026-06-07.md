...
3. 当前移动端横屏 `G4 Touch UI pass` 已通过：APK manifest 横屏、运行时禁用竖屏自动旋转、真机 smoke 截图为 2400x1080 横屏。
4. Android 等待期间完成的 PC1-PC57 前置包仍保留为证据；G5 性能预算已记录 Mi 11 Lite 基线，G6 已记录 iOS 的 Mac/Xcode/签名可行性边界，F2 已记录地图包/编辑器契约，F3 已记录 Web 排行/战绩公开边界，F4 已记录创作者经济/可选链上边界，F5 已记录服务器实现边界，F6 已搭起本地主服务器原型，F7 已写清 Unity-main-server 可选集成契约，F8 已落地可选 Unity 客户端适配器，正式下一步推进 F9 opt-in launch/debrief smoke 接线。
...
- G2-G6 已完成移动端可行性收口；F2-F8 平台契约、本地主服务器原型、Unity-main-server 集成契约和可选 Unity 客户端适配器已收口；F9 下一步只做 opt-in smoke 接线，仍不让当前 Unity Demo 强依赖远端服务。
...
| F9 | Next | `Wire optional Unity main-server adapter into launch/debrief smoke` | 只在 opt-in smoke 中请求签名小队和提交奖励 claim，默认离线演示不变 | smoke wiring + fallback gate |
...
- `north-patrol` steady-state sample: 30.48 FPS after 2 seconds warmup.
