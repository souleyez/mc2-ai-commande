...
| F19 | Next | `Capture opt-in post-receipt refresh evidence` | 捕获 opt-in post-debrief 主服务器 reward inventory refresh 的 debrief 文案、机库来源行、重复 claim 不重复应用和横版布局证据；默认离线、本地 fixture fallback、手机横版第一版和无逐帧服务端依赖保持不变 | evidence/gate |
