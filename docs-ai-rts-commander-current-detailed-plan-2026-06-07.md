...
| F51 | Next | `Implement post-F50 PC controlled-demo investor route evidence refresh audit fixes` | Make the F50 audit findings and follow-ups visible in plan, evidence and handoff surfaces before the next command-evidence metadata refresh. | PC investor route evidence audit fixes |
