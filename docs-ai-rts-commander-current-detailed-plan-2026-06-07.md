...
| F42 | Next | `Implement post-F41 PC controlled-demo investor evidence package fixes` | Convert the F41 follow-ups into the next fix slice: compact investor report highlights, stronger public-safe proxy visual identity, clearer damage/ejection/debrief presentation, and a fast investor-evidence gate separate from heavy aggregate validation. | PC investor evidence package fixes |
