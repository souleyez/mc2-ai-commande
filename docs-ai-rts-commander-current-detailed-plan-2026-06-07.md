...
4. Android 等待期间完成的 PC1-PC57 前置包仍保留为证据；G5 性能预算已记录 Mi 11 Lite 基线，G6 已记录 iOS 的 Mac/Xcode/签名可行性边界，F2 已记录地图包/编辑器契约，F3 已记录 Web 排行/战绩公开边界，F4 已记录创作者经济/可选链上边界，F5 已记录服务器实现边界，F6 已搭起本地主服务器原型，F7 已写清 Unity-main-server 可选集成契约，F8 已落地可选 Unity 客户端适配器，F9 已接通 opt-in launch/debrief smoke，正式下一步推进 F10 inventory bootstrap smoke 接线。
...
- G2-G6 已完成移动端可行性收口；F2-F9 平台契约、本地主服务器原型、Unity-main-server 集成契约、可选 Unity 客户端适配器和 opt-in launch/debrief smoke 已收口；F10 下一步只做 opt-in inventory bootstrap smoke，仍不让当前 Unity Demo 强依赖远端服务。
...
| F10 | Next | `Wire optional Unity inventory bootstrap smoke` | 只在 opt-in smoke 中拉取 dev account/inventory bootstrap，默认离线演示不变 | bootstrap smoke + fallback gate |
