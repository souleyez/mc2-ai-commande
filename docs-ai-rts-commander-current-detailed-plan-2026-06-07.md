...
| F27 | Next | `Audit Android entity placeholder collision path` | 审计仍保留 CreatePrimitive 的 fallback ground、单位、建筑和地形 prop 路径，明确哪些需要真实物理/碰撞占位，哪些应迁移到显式 mesh+collider，避免后续 Android logcat 噪声和碰撞需求混在一起 | Android entity placeholder collision audit |
