...
- Public replacement content slice 已打开，`Polish MechLab grid feel`、`Prove loadout battle effects` 和 `Polish weapon and damage readability` 已完成，下一步是 `Guard sparse battle UI regression`.
...
| Demo handoff 已刷新 | Full refresh 已验证 build、smoke、capture 和 boundary；V4 contact-spread evidence 已纳入证据页 | Guard sparse battle UI regression |
...
| 20 | Done | `Prove loadout battle effects` | G3 MechLab feel |
| 21 | Done | `Polish weapon and damage readability` | G1 combat feel |
| 22 | Next | `Guard sparse battle UI regression` | G1/G2 UI readability |
