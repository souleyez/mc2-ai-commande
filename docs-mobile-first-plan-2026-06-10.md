...
plan/preflight consistency check. The real G3 device smoke is still waiting on a physical Android
...
sealed through PC35, including the PC core playable contract check, mobile
...
Android APK payload check, Android APK size budget check, Android smoke artifact hygiene check, Android smoke screenshot evidence capture, Android smoke summary evidence output, Android smoke summary schema check, Android smoke summary preflight check, Android smoke plan/preflight consistency check, current plan gate
...
2. If no device row is shown, `check_android_device_preflight.ps1 -AllowNoDevice` should still prove the APK/tooling/package/summary-schema path and then stop at waiting-on-device; `check_android_smoke_plan_consistency.ps1` should also prove plan/preflight agreement; connect/authorize a phone before real G3.
