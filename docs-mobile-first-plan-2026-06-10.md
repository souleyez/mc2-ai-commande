...
sealed through PC23, including the PC core playable contract check, mobile
...
Android APK freshness check, Android APK identity check, current plan gate
...
- Android APK exists at `unity-mc2-demo\Builds\Android\MC2UnityDemo.apk` and passes `check_android_apk_freshness.ps1` plus `check_android_apk_identity.ps1`.
...
| G3-R1a | Device-smoke preflight can prove APK/tooling/package readiness before install | preflight rows | `check_android_device_preflight.ps1 -AllowNoDevice` reports waiting on device, with APK, APK freshness, APK identity, adb, aapt, package and activity OK |
