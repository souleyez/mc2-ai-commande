...
install-target metadata and Unity/IL2CPP runtime payload. The real G3 device
smoke is still waiting on a physical Android phone that is visible through `adb
devices` and authorized for USB debugging.
...
sealed through PC27, including the PC core playable contract check, mobile
...
Android APK payload check, current plan gate check, Android smoke log crash
...
- Android APK exists at `unity-mc2-demo\Builds\Android\MC2UnityDemo.apk` and passes `check_android_apk_freshness.ps1`, `check_android_apk_identity.ps1`, `check_android_apk_compatibility.ps1`, `check_android_apk_signing.ps1`, `check_android_apk_manifest.ps1` and `check_android_apk_payload.ps1`.
...
| G3-R1a | Device-smoke preflight can prove APK/tooling/package readiness before install | preflight rows | `check_android_device_preflight.ps1 -AllowNoDevice` reports waiting on device, with APK, APK freshness, APK identity, APK compatibility, APK signing, APK manifest, APK payload, adb, aapt, apksigner, package and activity OK |
