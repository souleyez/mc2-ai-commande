...
task is `F15 plan server-backed receipt slice`.
...
Next formal task -> F15 plan server-backed receipt slice.
```
...
- F15 plan server-backed receipt slice. This is the next formal task and must keep server authority out of the BattleCore frame loop.
