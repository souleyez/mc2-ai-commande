...
- Battle readability pass has begun with family-specific hit cues: energy beam plus pillar/ring, staggered missile salvo-spread arcs plus blast/smoke, and ballistic tracer plus punch/sparks.
- The combat situation smoke now guards the current weapon FX cue contract with `Energy=beam+pillar+muzzle Missile=arc+blast+salvo-spread Ballistic=tracer+sparks+muzzle+punch`.
...
- Missile, ballistic, and energy attacks now have clearer family-specific muzzle, trace, and hit accents layered over the existing primitive combat effects, with missiles using staggered salvo-spread arcs and ballistic hits using punch rings.
