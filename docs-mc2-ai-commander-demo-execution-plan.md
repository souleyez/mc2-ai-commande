...
- the fit Apply action labels itself Done, Invalid, Stock, or Apply so disabled apply states do not rely only on the status line
...
- The fit Apply control now reads Done, Invalid, Stock, or Apply depending on draft, validation, and inventory state.
