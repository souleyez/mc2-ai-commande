...
- focused mech bay fitting view with compact squad buttons, draft markers, and a selected-mech status line so one selected payload grid owns the drawer
...
- The squad fitting selector now marks draft mechs and shows the selected mech's pilot, fit state, and structure in the same compact strip.
