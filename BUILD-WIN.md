...
`F16 implement server-backed receipt evidence gate` is complete; `F17 plan post-receipt inventory refresh boundary` is complete; the formal next task is `F18 implement opt-in post-receipt inventory refresh binding`.
