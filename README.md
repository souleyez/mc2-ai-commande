...
继续作为主引擎。Android APK build smoke 已经通过；当前 Mi 11 Lite 已被 ADB
识别为授权 `device`，但首次 G3 安装被手机端策略拦截为
`INSTALL_FAILED_USER_RESTRICTED: Install canceled by user`。放开手机侧“通过 USB
安装/USB 调试安全设置”后继续真实 G3 smoke；`-AllowWaiting` 入口会把该状态
标记为 `G3InstallPolicyBlocked: True`，不把它误报成 G3 通过。在此之前，PC1-PC57 已先完成当前 PC 端
...
计划队列明确保留 `G3 Run Android device smoke` 为正式下一步；手机侧安装权限放开后再回到 Android 真机验证、触控指挥 UI 和移动端性能预算。
...
package is sealed through `PC1-PC57`, that `Add Android ADB driver package probe`
...
`winusb.inf` and adb exposes one authorized `device` row, so the remaining G3
blocker is phone-side USB install permission.
