...
已授权 USB 调试的 Android 手机。设备未到位时，PC1-PC20 已先完成当前 PC 端
受控展示包：Windows 构建、启动预检、证据健康检查、公开边界预检、
演示总预检、交接一致性检查、Android 真机 smoke 前置检查、PC 核心玩法合约检查、
移动指挥模型预检、战斗 HUD 稀疏合约检查、源码/生成物卫生检查、AI 副官边界检查、Windows 演示构建新鲜度检查、受控演示证据新鲜度检查、当前计划 gate 总预检、Android smoke 日志崩溃扫描、Android smoke 预演模式、visible-flow、截图证据、战场可读性和 MechLab 操作。
