...
手机端第一版固定按横屏版设计和验证，竖屏不是当前支持目标。Android 真机截图
...
默认关闭、失败回退的 Unity 主服务器客户端适配层；正式下一开发任务推进到
...
`F8 implement optional Unity main-server client adapter` are recorded, and
that `F9 wire optional Unity main-server adapter into launch/debrief smoke` is
