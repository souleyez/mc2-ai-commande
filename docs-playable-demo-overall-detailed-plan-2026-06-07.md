...
**Revision:** 2026-06-07 overall detailed plan v4. The current fixed overall plan is now `docs-ai-rts-commander-overall-implementation-plan-2026-06-07.md`; the day-to-day queue remains `docs-playable-demo-current-execution-plan-2026-06-07.md`.
...
1. `docs-ai-rts-commander-overall-implementation-plan-2026-06-07.md`: 当前定格版整体计划，覆盖产品方向、当前阶段、架构边界、里程碑、近端任务和后续平台路线。
2. `docs-playable-demo-current-execution-plan-2026-06-07.md`: 当前唯一日常执行入口，从 `Current Commit Queue` 的第一个 `Next` 或 `In Progress` 开始。
3. `docs-playable-demo-v1-detailed-plan-2026-06-07.md`: V1 Demo 的历史细化计划，保留产品边界、架构边界、提交级任务和验收门。
4. `docs-playable-demo-overall-detailed-plan-2026-06-07.md`: 总计划历史上下文。
5. `docs-reference-visual-audit-2026-06-07.md`: 截图、sidecar、smoke、validator 证据。
6. `docs-ai-commander-directive-contract.md`: AI 副官 observation/directive 合同。
7. `docs-content-replacement-plan.md` 和 `docs-content-pack.md`: 私有参考包与公开替换包边界。
8. `docs-platform-ecosystem-plan.md`: 地图服务器、排行、奖励认证、创作者生态、链上分账等后续平台方向。
...
| W8 交付包 | 协作者能构建、运行、演示、解释内容边界 | BUILD/README/证据页/边界检查齐全 | H1-H3, P1, P2 and H4 done; V4 polish next | walkthrough + build docs + evidence page |
...
- 当前下一步：`V4 Polish crowded contact occupancy`.
...
| --- | --- | --- |
| Demo handoff 已审计 | P1/P2/H4 已补 README、内容包合同、检查脚本和 handoff gate 记录 | V4 crowded contact polish |
| 碰撞占位后续只需回归 | V3 已提供单位、结构、hardProp 和 landing blocked 审计层；后续发现具体碰撞 bug 再加 close-up preset | V3 regression |
| 公开内容安全还要干净替换包 | 本地参考包可以开发验证，当前开发 build 会被检查正确标为 development-only | R1 replacement slice later |
...
这些都等 V4 crowded contact polish 和后续 replacement slice 收口后再开新阶段计划。
...
**Status:** Completed for handoff. H1 walkthrough, H2 repeatable Windows build, H3 evidence packaging, P1 content-boundary documentation, P2 public-boundary check and H4 handoff audit completed 2026-06-07.
...
6. Done: run a full handoff audit across build, smoke, captures and boundary status.
...
| 12 | Done | `Run demo handoff gate audit` | Validate build, smoke, captures and public boundary status together |
| 13 | Next | `Polish crowded contact occupancy` | Improve the densest combat screenshot with BattleCore evidence |
