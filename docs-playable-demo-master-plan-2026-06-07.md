...
**Current detailed execution entry:** `docs-playable-demo-current-execution-plan-2026-06-07.md` is now the preferred day-to-day plan. `docs-playable-demo-current-detailed-plan-2026-06-07.md` and `docs-playable-demo-detailed-execution-plan-2026-06-07.md` remain as historical phase context.
...
- Phase B / B2 比例审计、B3 碰撞占位证据和 B4 固定镜头构图已经完成，sidecar 现在能报告单位/prop 分类比例、BattleCore 占位、地形 landing predicate 和 camera composition offset；Phase C / C1 command state、C2 status-row solo flow 和 C3 squad jet landing rules 已经完成，当前下一步进入 C4 最小战斗 UI。
...
| Phase C: 指挥战斗闭环 | Active | 默认全队、状态栏单选、独立命令、自动归队、喷射和最小战斗 UI 可稳定演示 | 继续 C4 minimal battle UI |
...
1. `Freeze minimal battle UI`：战斗中只保留状态栏、喷射、任务地图、暂停/系统，不堆信息。
2. `Differentiate weapon visual effects`：激光、导弹、弹道、爆炸至少有可分辨的命中方向和效果层次。
3. `Strengthen mech section damage cues`：断臂、瘫痪、驾驶舱逃生在世界和状态栏都看得见。
4. `Make mech lab grid item fitting explicit`：武器/装甲/散热器按整块格子占位，合法性即时反馈。
5. `Apply mech lab loadouts in battle`：装配结果进入 BattleCore，影响武器/热量/重量表现。
...
**Status:** Completed 2026-06-07. Validator now rejects hard terrain-object jump landings, and `mc2_01-squad-jet-partial.txt` proves per-mech partial success with one jumping unit and two ready units that stay still.
...
2. `Differentiate weapon visual effects`
3. `Strengthen mech section damage cues`
4. `Make mech lab grid item fitting explicit`
5. `Apply mech lab loadouts in battle`
6. `Hide save system from first demo flow`
7. `Tighten debrief and repair loop`
8. `Freeze AI commander observation contract`
9. `Add AI commander directive adapter`
10. `Document replaceable visual content packs`
