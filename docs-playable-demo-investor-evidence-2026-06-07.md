...
| Android device preflight | Waiting on Device | `scripts/unity/check_android_device_preflight.ps1 -AllowNoDevice` reports APK, adb, aapt, apksigner, package, launch activity, compatibility metadata and signing are ready, then stops at waiting on an authorized Android phone. |
