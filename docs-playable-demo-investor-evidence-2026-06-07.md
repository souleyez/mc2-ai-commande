...
| Android G3 when-ready runner | Blocked by phone policy | `scripts/unity/run_android_g3_when_ready.ps1 -PlanOnly` still reports `NoInstallOrLaunchUntilDeviceReady: True`; with the current authorized phone, `scripts/unity/run_android_g3_when_ready.ps1 -TimeoutSeconds 0 -AllowWaiting` reached `adb install` and failed with `INSTALL_FAILED_USER_RESTRICTED: Install canceled by user`; the APK package is not installed yet, so the next action is enabling phone-side USB installation and retrying G3. |
