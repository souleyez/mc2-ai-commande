...
| Current plan gate | Pass | `scripts/unity/check_current_plan_gate.ps1` reports `Current plan gate check OK` by wrapping handoff/readiness, demo source hygiene, mobile command model, battle HUD sparse contract and Android device-smoke preflight state in one command. |
...
| Android smoke plan | Pass | `scripts/unity/android_device_smoke.ps1 -PlanOnly` reports `Android device smoke plan OK`, resolving APK/tool/package/activity/log path and install/launch/log-check actions without requiring a connected phone. |
