...
| Current plan queue consistency | Pass | `scripts/unity/check_current_plan_queue.ps1` reports `Current plan queue consistency check OK` by proving README, BUILD-WIN, master/detailed/PC/mobile/evidence/handoff docs and helper scripts agree on `PC1-PC56`, `Add Android G3 when-ready runner`, `WpdOnlyAndroidProbe: True`, `AdbSetupHint: True`, `AdbWatchHint: True`, `G3DeviceStatusReport: True`, `G3WhenReady: True`, and `G3 Run Android device smoke` as the formal next task. |
| Android device connection | Pass | `scripts/unity/check_android_device_connection.ps1` reports `Android device connection check waiting on device`, `WpdOnlyAndroidProbe: True` and `AdbSetupHint: True`, and can distinguish no-device, unauthorized, offline, multi-device and ready states before G3 install/launch. |
...
| Android ADB setup guidance | Pass | The connection helper reports current Windows driver/provider/inf/service for MTP-only phones, including the current Mi 11 Lite `provider=Microsoft`, `inf=wpdmtp.inf` and `service=WUDFWpdMtp` state. |
| Android ADB readiness watch | Pass | `scripts/unity/watch_android_device_connection.ps1 -Once -AllowWaiting` reports `Android device connection watch waiting on device` and `AdbWatchHint: True`; normal timed mode can wait for one authorized adb `device` row without installing or launching the APK. |
| Android G3 device status report | Pass | `scripts/unity/write_android_g3_device_status.ps1` reports `Android G3 device status report OK`, `G3DeviceStatusReport: True`, `G3DeviceReady: False` and `NoInstallOrLaunch: True`, then writes ignored `analysis-output\android-g3-device-status.json` with the current blocker. |
| Android G3 when-ready runner | Pass | `scripts/unity/run_android_g3_when_ready.ps1 -PlanOnly` reports `Android G3 when-ready plan OK`, `G3WhenReady: True` and `NoInstallOrLaunchUntilDeviceReady: True`; `-TimeoutSeconds 0 -AllowWaiting` reports waiting while the phone is WPD/MTP-only and does not install or launch. |
| Current plan gate | Pass | `scripts/unity/check_current_plan_gate.ps1` reports `Current plan gate check OK` by wrapping handoff/readiness, Windows build freshness, demo source hygiene, AI deputy contract, mobile command model, battle HUD sparse contract, PC visual capture sanity, PC visual capture sanity self-test, PC capture sidecar schema, PC capture preset contract, PC capture artifact hygiene, PC window contract, PC launch log hygiene, PC build artifact hygiene, PC smoke artifact hygiene, current plan queue consistency, Android device connection, Android ADB readiness watch, Android G3 device status report, Android G3 when-ready runner and Android device-smoke preflight state in one command. |
...
| Android smoke connection gate check | Pass | `scripts/unity/check_android_smoke_connection_gate.ps1` reports `Android smoke connection gate check OK` and, on the current no-phone machine, `Android smoke connection gate check waiting on device`; the check proves the real smoke fail-fast path without rewriting smoke log, screenshot or summary evidence. |
...
| Android G3 device requirement | Waiting on Device | `scripts/unity/check_android_g3_device_requirement.ps1` reports `Android G3 device requirement check waiting on device`, proving strict G3 readiness still requires an authorized Android phone. |
