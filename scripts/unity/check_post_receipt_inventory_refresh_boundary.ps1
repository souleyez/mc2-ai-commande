...
foreach ($doc in $docs) {
    ...
    Require-Text -Text $doc.Text -Needle "F22 audit landscape MechLab touch controls" -Label "$($doc.Name) F22 next marker"
}
...
Require-Text -Text $masterPlan -Needle '| 100 | Next | `Audit landscape MechLab touch controls` |' -Label "master F22 next"
...
Require-Text -Text $detailedPlan -Needle '| F22 | Next | `Audit landscape MechLab touch controls` |' -Label "detailed F22 next"
...
Require-Text -Text $queue -Needle "F22 audit landscape MechLab touch controls" -Label "queue next F22"
