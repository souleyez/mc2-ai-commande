...
foreach ($doc in $docs) {
    ...
    Require-Text -Text $doc.Text -Needle "F24 capture Android MechLab touch evidence" -Label "$($doc.Name) F24 next marker"
}
...
Require-Text -Text $queue -Needle "F24 capture Android MechLab touch evidence" -Label "queue next F24"
