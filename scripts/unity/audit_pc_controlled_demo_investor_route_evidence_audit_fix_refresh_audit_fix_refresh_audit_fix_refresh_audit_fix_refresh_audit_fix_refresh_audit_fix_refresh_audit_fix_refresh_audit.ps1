...
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $OutputDir = Join-Path $RepoRoot "analysis-output\pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit"
}
...
$f67RefreshReportRel = "analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh/report.json"
$f67RefreshMarkdownRel = "analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh/report.md"
$f65AuditReportRel = "analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit/report.json"
$f66FixReportRel = "analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fixes/report.json"
...
if ($PlanOnly) {
    Write-Host "PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit plan OK."
...
$f67RefreshReport = Read-RepoJson -RelativePath $f67RefreshReportRel
$f67RefreshMarkdown = Read-RepoText -RelativePath $f67RefreshMarkdownRel
$f65AuditReport = Read-RepoJson -RelativePath $f65AuditReportRel
$f66FixReport = Read-RepoJson -RelativePath $f66FixReportRel
...
$f68AuditMarkers = @(
    "RouteAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAudit=pass-with-followups source=$f68AuditReportRel completed=F68 next=F69 noUnityLaunch=True mobile=landscape-only",
...
Assert-All -Text $commandCaptureScript -Label "command capture F67/F68 source contract" -Needles @(
...
Assert-All -Text $commandCaptureScript -Label "command capture F67 closure source" -Needles $f67ClosureMarkers
...
Assert-All -Text $commandMarkdown -Label "command markdown F67 closure markers" -Needles $f67ClosureMarkers

Assert-All -Text $f67RefreshMarkdown -Label "F67 refresh gate markdown" -Needles @(
    'Completed task: `F67 refresh PC controlled-demo investor route evidence after F65 audit fixes`',
    'Next formal task: `F68 audit post-F67 PC controlled-demo investor route evidence refresh`',
    'NoUnityLaunch: True',
...
if ($null -ne $commandReport) {
    ...
    Assert-Equals -Actual ([string]$commandReport.sourceRouteAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditReport) -Expected $f65AuditReportRel -Label "command report source F65 audit"
    Assert-Equals -Actual ([string]$commandReport.sourceRouteAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixesReport) -Expected $f66FixReportRel -Label "command report source F66 fixes"
    ...
    foreach ($marker in $f67ClosureMarkers) {
...
if ($null -ne $f67RefreshReport) {
    Assert-Equals -Actual ([string]$f67RefreshReport.schema) -Expected "PCControlledDemoInvestorRouteEvidenceAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefresh" -Label "F67 refresh report schema"
    Assert-Equals -Actual ([string]$f67RefreshReport.result) -Expected "pass" -Label "F67 refresh report result"
    Assert-Equals -Actual ([string]$f67RefreshReport.completedTask) -Expected "F67 refresh PC controlled-demo investor route evidence after F65 audit fixes" -Label "F67 refresh completed task"
    Assert-Equals -Actual ([string]$f67RefreshReport.nextFormalTask) -Expected "F68 audit post-F67 PC controlled-demo investor route evidence refresh" -Label "F67 refresh next task"
    Assert-Equals -Actual ([bool]$f67RefreshReport.noUnityLaunch) -Expected $true -Label "F67 refresh no Unity launch"
    Assert-Equals -Actual ([string]$f67RefreshReport.sourceAuditReport) -Expected $f65AuditReportRel -Label "F67 source F65 audit"
    Assert-Equals -Actual ([string]$f67RefreshReport.sourceFixesReport) -Expected $f66FixReportRel -Label "F67 source F66 fixes"
}
...
if ($null -ne $f66FixReport) {
    Assert-Equals -Actual ([string]$f66FixReport.result) -Expected "pass" -Label "F66 fixes result"
    Assert-Equals -Actual ([string]$f66FixReport.completedTask) -Expected "F66 implement post-F65 PC controlled-demo investor route evidence refresh audit fixes" -Label "F66 fixes completed task"
    Assert-Equals -Actual ([string]$f66FixReport.nextFormalTask) -Expected "F67 refresh PC controlled-demo investor route evidence after F65 audit fixes" -Label "F66 fixes next task"
}

Assert-All -Text $masterPlan -Label "master F68/F69 queue" -Needles @(
    'PC1-PC68',
    '| 146 | Done | `Audit post-F67 PC controlled-demo investor route evidence refresh` |',
    '| 147 | Next | `Implement post-F68 PC controlled-demo investor route evidence refresh audit fixes` |',
    'Mobile phones remain landscape-only for the first playable target'
)
Assert-All -Text $detailedPlan -Label "detailed F68/F69 queue" -Needles @(
    '| F68 | Done | `Audit post-F67 PC controlled-demo investor route evidence refresh` |',
    '| F69 | Next | `Implement post-F68 PC controlled-demo investor route evidence refresh audit fixes` |'
)
Assert-All -Text $mobilePlan -Label "mobile F68/F69 status" -Needles @(
    'PC1-PC68',
    'F68 audit post-F67 PC controlled-demo investor route evidence refresh',
...
Assert-All -Text $handoffDoc -Label "handoff F69 next" -Needles @(
    'Latest sealed PC/mobile wait-state checkpoint: `PC1-PC68`',
    'Current formal next development task after handoff: `F69 implement post-F68 PC controlled-demo investor route evidence refresh audit fixes`',
    'Next planned work: `F69 implement post-F68 PC controlled-demo investor route evidence refresh audit fixes`',
    'first phone version is landscape-only'
)
Assert-All -Text $routeDoc -Label "route doc F68 audit visibility" -Needles $f68AuditMarkers
Assert-All -Text $evidenceDoc -Label "evidence doc F68 audit visibility" -Needles $f68AuditMarkers
Assert-All -Text $queueScript -Label "queue F68/F69 markers" -Needles @(
    'PC1-PC68',
    'F68 audit post-F67 PC controlled-demo investor route evidence refresh',
    'F69 implement post-F68 PC controlled-demo investor route evidence refresh audit fixes',
    '| F68 | Done | `Audit post-F67 PC controlled-demo investor route evidence refresh` |',
...
Assert-All -Text $currentGate -Label "current gate F68 audit plan" -Needles @(
    'audit_pc_controlled_demo_investor_route_evidence_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit.ps1',
    'PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit plan OK.'
)
Assert-All -Text $handoffScript -Label "handoff script F68/F69 markers" -Needles @(
    'audit_pc_controlled_demo_investor_route_evidence_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit.ps1',
    'PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit OK',
    'F69 implement post-F68 PC controlled-demo investor route evidence refresh audit fixes'
)
Assert-Contains -Text $gitignore -Needle 'analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit/' -Label ".gitignore F68 output"

Add-Finding -Area "route-proof-clarity" -Status "pass" -Detail "F67 command evidence keeps the five-preset Windows investor route and links command report, screenshots, sidecars and logs"
Add-Finding -Area "damage-ejection-proof" -Status "pass" -Detail "damage-demo still carries section loss, cockpit ejection, wreck salvage and repair-cost proof after the F67 refresh"
Add-Finding -Area "mobile-landscape-proof" -Status "pass" -Detail "F67 command evidence and plan surfaces keep first phone version landscape-only as a horizontal phone build"
...
Add-Finding -Area "F67-refresh-traceability" -Status "pass" -Detail "F67 command report and markdown carry F65 audit and F66 fix reports as explicit sources"

Add-FollowUp -Priority "P1" -Area "audit-visibility" -Issue "F68 audit findings should be made visible in plan, evidence and handoff surfaces before the next refresh step." -NextFix "Implement F69 as a narrow doc/gate update that records F68 audit findings, keeps no-Unity-launch evidence, and preserves the landscape-only phone scope."
Add-FollowUp -Priority "P2" -Area "next-refresh-contract" -Issue "The next command-evidence refresh should consume the F68 audit report explicitly so the audit closure can be traced." -NextFix "When F69 is complete, require its ignored report before advancing command evidence metadata again."

if ($failures.Count -gt 0) {
    Write-Host "PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit failed."
    foreach ($failure in $failures) {
        Write-Host " - $failure"
    }

    throw "$($failures.Count) PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit check(s) failed."
}

New-Item -ItemType Directory -Force -Path $OutputDir | Out-Null
$report = [pscustomobject]@{
    schema = "PCControlledDemoInvestorRouteEvidenceAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAudit"
    result = "pass-with-followups"
    timestampUtc = (Get-Date).ToUniversalTime().ToString("o")
    completedTask = "F68 audit post-F67 PC controlled-demo investor route evidence refresh"
    nextFormalTask = "F69 implement post-F68 PC controlled-demo investor route evidence refresh audit fixes"
    sourceCommandEvidenceReport = "analysis-output/pc-controlled-demo-command-evidence/pc-controlled-demo-command-evidence.json"
    sourceF67RefreshReport = $f67RefreshReportRel
    sourceF65AuditReport = $f65AuditReportRel
    sourceF66FixReport = $f66FixReportRel
    noUnityLaunch = $true
...
$markdownLines.Add("# PC Controlled Demo Investor Route Evidence Audit Fix Refresh Audit Fix Refresh Audit Fix Refresh Audit Fix Refresh Audit Fix Refresh Audit Fix Refresh Audit Fix Refresh Audit")
$markdownLines.Add("")
$markdownLines.Add("Result: pass-with-followups")
$markdownLines.Add("")
$markdownLines.Add('Completed task: `F68 audit post-F67 PC controlled-demo investor route evidence refresh`')
$markdownLines.Add('Next formal task: `F69 implement post-F68 PC controlled-demo investor route evidence refresh audit fixes`')
$markdownLines.Add("")
...
$markdownLines.Add("Source F67 refresh: ``$f67RefreshReportRel``")
$markdownLines.Add("Source F65 audit: ``$f65AuditReportRel``")
$markdownLines.Add("Source F66 fixes: ``$f66FixReportRel``")
$markdownLines.Add("")
...
Write-Host "PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit OK."
