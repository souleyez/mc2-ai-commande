...
Assert-Contains -Text $mobilePlan -Needle "F63 implement post-F62 PC controlled-demo investor route evidence refresh audit fixes" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F64 refresh PC controlled-demo investor route evidence after F62 audit fixes" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F65 audit post-F64 PC controlled-demo investor route evidence refresh" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F66 implement post-F65 PC controlled-demo investor route evidence refresh audit fixes" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F67 refresh PC controlled-demo investor route evidence after F65 audit fixes" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F68 audit post-F67 PC controlled-demo investor route evidence refresh" -Label "mobile completed platform task"
...
Assert-Contains -Text $mobilePlan -Needle "F73 refresh PC controlled-demo investor route evidence after F71 audit fixes" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F74 audit post-F73 PC controlled-demo investor route evidence refresh" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F75 implement post-F74 PC controlled-demo investor route evidence refresh audit fixes" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F76 refresh PC controlled-demo investor route evidence after F74 audit fixes" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F77 audit post-F76 PC controlled-demo investor route evidence refresh" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F78 implement post-F77 PC controlled-demo investor route evidence refresh audit fixes" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F79 refresh PC controlled-demo investor route evidence after F77 audit fixes" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F80 audit post-F79 PC controlled-demo investor route evidence refresh" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F81 implement post-F80 PC controlled-demo investor route evidence refresh audit fixes" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F82 refresh PC controlled-demo investor route evidence after F80 audit fixes" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F83 audit post-F82 PC controlled-demo investor route evidence refresh" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F84 implement post-F83 PC controlled-demo investor route evidence refresh audit fixes" -Label "mobile completed platform task"
Assert-Contains -Text $mobilePlan -Needle "F85 refresh PC controlled-demo investor route evidence after F83 audit fixes" -Label "mobile next task"
...
Assert-Contains -Text $detailedPlan -Needle '| F63 | Done | `Implement post-F62 PC controlled-demo investor route evidence refresh audit fixes` |' -Label "detailed queue F63"
Assert-Contains -Text $detailedPlan -Needle '| F64 | Done | `Refresh PC controlled-demo investor route evidence after F62 audit fixes` |' -Label "detailed queue F64"
Assert-Contains -Text $detailedPlan -Needle '| F65 | Done | `Audit post-F64 PC controlled-demo investor route evidence refresh` |' -Label "detailed queue F65"
Assert-Contains -Text $detailedPlan -Needle '| F66 | Done | `Implement post-F65 PC controlled-demo investor route evidence refresh audit fixes` |' -Label "detailed queue F66"
Assert-Contains -Text $detailedPlan -Needle '| F67 | Done | `Refresh PC controlled-demo investor route evidence after F65 audit fixes` |' -Label "detailed queue F67"
Assert-Contains -Text $detailedPlan -Needle '| F68 | Done | `Audit post-F67 PC controlled-demo investor route evidence refresh` |' -Label "detailed queue F68"
...
Assert-Contains -Text $detailedPlan -Needle '| F73 | Done | `Refresh PC controlled-demo investor route evidence after F71 audit fixes` |' -Label "detailed queue F73"
Assert-Contains -Text $detailedPlan -Needle '| F74 | Done | `Audit post-F73 PC controlled-demo investor route evidence refresh` |' -Label "detailed queue F74"
Assert-Contains -Text $detailedPlan -Needle '| F75 | Done | `Implement post-F74 PC controlled-demo investor route evidence refresh audit fixes` |' -Label "detailed queue F75"
Assert-Contains -Text $detailedPlan -Needle '| F76 | Done | `Refresh PC controlled-demo investor route evidence after F74 audit fixes` |' -Label "detailed queue F76"
Assert-Contains -Text $detailedPlan -Needle '| F77 | Done | `Audit post-F76 PC controlled-demo investor route evidence refresh` |' -Label "detailed queue F77"
Assert-Contains -Text $detailedPlan -Needle '| F78 | Done | `Implement post-F77 PC controlled-demo investor route evidence refresh audit fixes` |' -Label "detailed queue F78"
Assert-Contains -Text $detailedPlan -Needle '| F79 | Done | `Refresh PC controlled-demo investor route evidence after F77 audit fixes` |' -Label "detailed queue F79"
Assert-Contains -Text $detailedPlan -Needle '| F80 | Done | `Audit post-F79 PC controlled-demo investor route evidence refresh` |' -Label "detailed queue F80"
Assert-Contains -Text $detailedPlan -Needle '| F81 | Done | `Implement post-F80 PC controlled-demo investor route evidence refresh audit fixes` |' -Label "detailed queue F81"
Assert-Contains -Text $detailedPlan -Needle '| F82 | Done | `Refresh PC controlled-demo investor route evidence after F80 audit fixes` |' -Label "detailed queue F82"
Assert-Contains -Text $detailedPlan -Needle '| F83 | Done | `Audit post-F82 PC controlled-demo investor route evidence refresh` |' -Label "detailed queue F83"
Assert-Contains -Text $detailedPlan -Needle '| F84 | Done | `Implement post-F83 PC controlled-demo investor route evidence refresh audit fixes` |' -Label "detailed queue F84"
Assert-Contains -Text $detailedPlan -Needle '| F85 | Next | `Refresh PC controlled-demo investor route evidence after F83 audit fixes` |' -Label "detailed queue F85"

$handoff = Read-RequiredText -RelativePath "docs-machine-handoff-plan-2026-06-07.md"
Assert-Contains -Text $handoff -Needle 'Current formal next development task after handoff: `F85 refresh PC controlled-demo investor route evidence after F83 audit fixes`' -Label "handoff next task"
Assert-Contains -Text $handoff -Needle 'Next planned work: `F85 refresh PC controlled-demo investor route evidence after F83 audit fixes`' -Label "handoff next planned work"
...
Assert-Contains -Text $currentGate -Needle 'check_f81_pc_route_audit_fixes.ps1' -Label "current gate F81 script marker"
Assert-Contains -Text $currentGate -Needle 'F81 PC route audit fixes plan OK.' -Label "current gate F81 plan marker"
Assert-Contains -Text $currentGate -Needle 'check_f82_pc_route_evidence_refresh.ps1' -Label "current gate F82 script marker"
Assert-Contains -Text $currentGate -Needle 'F82 PC route evidence refresh plan OK.' -Label "current gate F82 plan marker"
Assert-Contains -Text $currentGate -Needle 'audit_f83_pc_route_evidence_refresh.ps1' -Label "current gate F83 script marker"
Assert-Contains -Text $currentGate -Needle 'F83 PC route evidence refresh audit plan OK.' -Label "current gate F83 plan marker"
Assert-Contains -Text $currentGate -Needle 'check_f84_pc_route_audit_fixes.ps1' -Label "current gate F84 script marker"
Assert-Contains -Text $currentGate -Needle 'F84 PC route audit fixes plan OK.' -Label "current gate F84 plan marker"
...
Assert-Contains -Text $currentGate -Needle 'audit_f74_pc_route_evidence_refresh.ps1' -Label "current gate F74 route evidence audit script marker"
Assert-Contains -Text $currentGate -Needle 'F74 PC route evidence refresh audit plan OK.' -Label "current gate F74 route evidence audit plan marker"
Assert-Contains -Text $currentGate -Needle 'check_f75_pc_route_audit_fixes.ps1' -Label "current gate F75 route audit fixes script marker"
Assert-Contains -Text $currentGate -Needle 'F75 PC route audit fixes plan OK.' -Label "current gate F75 route audit fixes plan marker"
Assert-Contains -Text $currentGate -Needle 'check_f76_pc_route_evidence_refresh.ps1' -Label "current gate F76 route evidence refresh script marker"
Assert-Contains -Text $currentGate -Needle 'F76 PC route evidence refresh plan OK.' -Label "current gate F76 route evidence refresh plan marker"
Assert-Contains -Text $currentGate -Needle 'audit_f77_pc_route_evidence_refresh.ps1' -Label "current gate F77 route evidence audit script marker"
Assert-Contains -Text $currentGate -Needle 'F77 PC route evidence refresh audit plan OK.' -Label "current gate F77 route evidence audit plan marker"
Assert-Contains -Text $currentGate -Needle 'check_f78_pc_route_audit_fixes.ps1' -Label "current gate F78 route audit fixes script marker"
Assert-Contains -Text $currentGate -Needle 'F78 PC route audit fixes plan OK.' -Label "current gate F78 route audit fixes plan marker"
Assert-Contains -Text $currentGate -Needle 'check_f79_pc_route_evidence_refresh.ps1' -Label "current gate F79 route evidence refresh script marker"
Assert-Contains -Text $currentGate -Needle 'F79 PC route evidence refresh plan OK.' -Label "current gate F79 route evidence refresh plan marker"
Assert-Contains -Text $currentGate -Needle 'audit_f80_pc_route_evidence_refresh.ps1' -Label "current gate F80 route evidence audit script marker"
Assert-Contains -Text $currentGate -Needle 'F80 PC route evidence refresh audit plan OK.' -Label "current gate F80 route evidence audit plan marker"
...
Assert-Contains -Text $handoffScript -Needle 'F63 implement post-F62 PC controlled-demo investor route evidence refresh audit fixes' -Label "handoff script F63 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_pc_controlled_demo_investor_route_evidence_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fixes.ps1' -Label "handoff script F63 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fixes script marker"
Assert-Contains -Text $handoffScript -Needle 'PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fixes check OK' -Label "handoff script F63 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fixes marker"
Assert-Contains -Text $handoffScript -Needle 'F64 refresh PC controlled-demo investor route evidence after F62 audit fixes' -Label "handoff script F64 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_pc_controlled_demo_investor_route_evidence_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh.ps1' -Label "handoff script F64 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh script marker"
Assert-Contains -Text $handoffScript -Needle 'PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh check OK' -Label "handoff script F64 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh marker"
Assert-Contains -Text $handoffScript -Needle 'F65 audit post-F64 PC controlled-demo investor route evidence refresh' -Label "handoff script F65 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'audit_pc_controlled_demo_investor_route_evidence_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit.ps1' -Label "handoff script F65 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit script marker"
Assert-Contains -Text $handoffScript -Needle 'PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit OK' -Label "handoff script F65 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit marker"
Assert-Contains -Text $handoffScript -Needle 'F66 implement post-F65 PC controlled-demo investor route evidence refresh audit fixes' -Label "handoff script F66 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_pc_controlled_demo_investor_route_evidence_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fixes.ps1' -Label "handoff script F66 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fixes script marker"
Assert-Contains -Text $handoffScript -Needle 'PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fixes check OK' -Label "handoff script F66 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fixes marker"
Assert-Contains -Text $handoffScript -Needle 'F67 refresh PC controlled-demo investor route evidence after F65 audit fixes' -Label "handoff script F67 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_pc_controlled_demo_investor_route_evidence_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh.ps1' -Label "handoff script F67 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh script marker"
Assert-Contains -Text $handoffScript -Needle 'PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh check OK' -Label "handoff script F67 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh marker"
Assert-Contains -Text $handoffScript -Needle 'F68 audit post-F67 PC controlled-demo investor route evidence refresh' -Label "handoff script F68 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'audit_pc_controlled_demo_investor_route_evidence_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit.ps1' -Label "handoff script F68 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit script marker"
Assert-Contains -Text $handoffScript -Needle 'PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit OK' -Label "handoff script F68 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit marker"
Assert-Contains -Text $handoffScript -Needle 'F69 implement post-F68 PC controlled-demo investor route evidence refresh audit fixes' -Label "handoff script F69 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_pc_controlled_demo_investor_route_evidence_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fixes.ps1' -Label "handoff script F69 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fixes script marker"
Assert-Contains -Text $handoffScript -Needle 'PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fixes check OK' -Label "handoff script F69 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fixes marker"
Assert-Contains -Text $handoffScript -Needle 'F70 refresh PC controlled-demo investor route evidence after F68 audit fixes' -Label "handoff script F70 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_f70_pc_route_evidence_refresh.ps1' -Label "handoff script F70 investor route evidence refresh script marker"
Assert-Contains -Text $handoffScript -Needle 'PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh check OK' -Label "handoff script F70 investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh marker"
...
Assert-Contains -Text $handoffScript -Needle 'F73 refresh PC controlled-demo investor route evidence after F71 audit fixes' -Label "handoff script F73 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_f73_pc_route_evidence_refresh.ps1' -Label "handoff script F73 investor route evidence refresh script marker"
Assert-Contains -Text $handoffScript -Needle 'F73 PC route evidence refresh check OK.' -Label "handoff script F73 investor route evidence refresh marker"
Assert-Contains -Text $handoffScript -Needle 'F74 audit post-F73 PC controlled-demo investor route evidence refresh' -Label "handoff script F74 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'audit_f74_pc_route_evidence_refresh.ps1' -Label "handoff script F74 investor route evidence refresh audit script marker"
Assert-Contains -Text $handoffScript -Needle 'F74 PC route evidence refresh audit OK.' -Label "handoff script F74 investor route evidence refresh audit marker"
Assert-Contains -Text $handoffScript -Needle 'F75 implement post-F74 PC controlled-demo investor route evidence refresh audit fixes' -Label "handoff script F75 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_f75_pc_route_audit_fixes.ps1' -Label "handoff script F75 investor route evidence audit fixes script marker"
Assert-Contains -Text $handoffScript -Needle 'F75 PC route audit fixes check OK.' -Label "handoff script F75 investor route evidence audit fixes marker"
Assert-Contains -Text $handoffScript -Needle 'F76 refresh PC controlled-demo investor route evidence after F74 audit fixes' -Label "handoff script F76 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_f76_pc_route_evidence_refresh.ps1' -Label "handoff script F76 investor route evidence refresh script marker"
Assert-Contains -Text $handoffScript -Needle 'F76 PC route evidence refresh check OK.' -Label "handoff script F76 investor route evidence refresh marker"
Assert-Contains -Text $handoffScript -Needle 'F77 audit post-F76 PC controlled-demo investor route evidence refresh' -Label "handoff script F77 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'audit_f77_pc_route_evidence_refresh.ps1' -Label "handoff script F77 investor route evidence refresh audit script marker"
Assert-Contains -Text $handoffScript -Needle 'F77 PC route evidence refresh audit OK.' -Label "handoff script F77 investor route evidence refresh audit marker"
Assert-Contains -Text $handoffScript -Needle 'F78 implement post-F77 PC controlled-demo investor route evidence refresh audit fixes' -Label "handoff script F78 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_f78_pc_route_audit_fixes.ps1' -Label "handoff script F78 investor route audit fixes script marker"
Assert-Contains -Text $handoffScript -Needle 'F78 PC route audit fixes check OK.' -Label "handoff script F78 investor route audit fixes marker"
Assert-Contains -Text $handoffScript -Needle 'F79 refresh PC controlled-demo investor route evidence after F77 audit fixes' -Label "handoff script F79 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_f79_pc_route_evidence_refresh.ps1' -Label "handoff script F79 investor route evidence refresh script marker"
Assert-Contains -Text $handoffScript -Needle 'F79 PC route evidence refresh check OK.' -Label "handoff script F79 investor route evidence refresh marker"
Assert-Contains -Text $handoffScript -Needle 'F80 audit post-F79 PC controlled-demo investor route evidence refresh' -Label "handoff script F80 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'audit_f80_pc_route_evidence_refresh.ps1' -Label "handoff script F80 investor route evidence audit script marker"
Assert-Contains -Text $handoffScript -Needle 'F80 PC route evidence refresh audit OK.' -Label "handoff script F80 investor route evidence audit marker"
Assert-Contains -Text $handoffScript -Needle 'F81 implement post-F80 PC controlled-demo investor route evidence refresh audit fixes' -Label "handoff script F81 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_f81_pc_route_audit_fixes.ps1' -Label "handoff script F81 investor route audit fixes script marker"
Assert-Contains -Text $handoffScript -Needle 'F81 PC route audit fixes check OK.' -Label "handoff script F81 investor route audit fixes marker"
Assert-Contains -Text $handoffScript -Needle 'F82 refresh PC controlled-demo investor route evidence after F80 audit fixes' -Label "handoff script F82 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_f82_pc_route_evidence_refresh.ps1' -Label "handoff script F82 investor route evidence refresh script marker"
Assert-Contains -Text $handoffScript -Needle 'F82 PC route evidence refresh check OK.' -Label "handoff script F82 investor route evidence refresh marker"
Assert-Contains -Text $handoffScript -Needle 'F83 audit post-F82 PC controlled-demo investor route evidence refresh' -Label "handoff script F83 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'audit_f83_pc_route_evidence_refresh.ps1' -Label "handoff script F83 investor route evidence refresh audit script marker"
Assert-Contains -Text $handoffScript -Needle 'F83 PC route evidence refresh audit OK.' -Label "handoff script F83 investor route evidence refresh audit marker"
Assert-Contains -Text $handoffScript -Needle 'F84 implement post-F83 PC controlled-demo investor route evidence refresh audit fixes' -Label "handoff script F84 completed task marker"
Assert-Contains -Text $handoffScript -Needle 'check_f84_pc_route_audit_fixes.ps1' -Label "handoff script F84 investor route audit fixes script marker"
Assert-Contains -Text $handoffScript -Needle 'F84 PC route audit fixes check OK.' -Label "handoff script F84 investor route audit fixes marker"
Assert-Contains -Text $handoffScript -Needle 'F85 refresh PC controlled-demo investor route evidence after F83 audit fixes' -Label "handoff script F85 next task marker"
