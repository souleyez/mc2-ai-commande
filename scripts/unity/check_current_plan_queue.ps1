...
Assert-Contains -Text $mobilePlan -Needle "F60 implement post-F59 PC controlled-demo investor route evidence refresh audit fixes" -Label "mobile next task"
...
Assert-Contains -Text $detailedPlan -Needle '| F60 | Next | `Implement post-F59 PC controlled-demo investor route evidence refresh audit fixes` |' -Label "detailed queue F60"

$handoff = Read-RequiredText -RelativePath "docs-machine-handoff-plan-2026-06-07.md"
Assert-Contains -Text $handoff -Needle 'Current formal next development task after handoff: `F60 implement post-F59 PC controlled-demo investor route evidence refresh audit fixes`' -Label "handoff next task"
Assert-Contains -Text $handoff -Needle 'Next planned work: `F60 implement post-F59 PC controlled-demo investor route evidence refresh audit fixes`' -Label "handoff next planned work"
...
Assert-Contains -Text $handoffScript -Needle 'F60 implement post-F59 PC controlled-demo investor route evidence refresh audit fixes' -Label "handoff script F60 next task marker"
