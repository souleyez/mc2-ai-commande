...
Assert-Contains -Text $mobilePlan -Needle "F51 implement post-F50 PC controlled-demo investor route evidence refresh audit fixes" -Label "mobile next task"
...
Assert-Contains -Text $detailedPlan -Needle '| F51 | Next | `Implement post-F50 PC controlled-demo investor route evidence refresh audit fixes` |' -Label "detailed queue F51"

$handoff = Read-RequiredText -RelativePath "docs-machine-handoff-plan-2026-06-07.md"
Assert-Contains -Text $handoff -Needle 'Current formal next development task after handoff: `F51 implement post-F50 PC controlled-demo investor route evidence refresh audit fixes`' -Label "handoff next task"
Assert-Contains -Text $handoff -Needle 'Next planned work: `F51 implement post-F50 PC controlled-demo investor route evidence refresh audit fixes`' -Label "handoff next planned work"
...
Assert-Contains -Text $handoffScript -Needle 'F51 implement post-F50 PC controlled-demo investor route evidence refresh audit fixes' -Label "handoff script F51 next task marker"
