...
Require-Text -Text $masterPlan -Needle '| 86 | Next | `Implement optional Unity main-server client adapter` |' -Label "master queue F8"
Require-Text -Text $detailedPlan -Needle "| F7 | Done | ``Document Unity main-server integration contract`` |" -Label "detailed queue F7"
Require-Text -Text $detailedPlan -Needle "| F8 | Next | ``Implement optional Unity main-server client adapter`` |" -Label "detailed queue F8"
Require-Text -Text $mobilePlan -Needle "F8 implement optional Unity main-server client adapter" -Label "mobile next task"
Require-Text -Text $handoff -Needle 'Current formal next development task after handoff: `F8 implement optional Unity main-server client adapter`' -Label "handoff next task"
Require-Text -Text $handoff -Needle 'Next planned work: `F8 implement optional Unity main-server client adapter`' -Label "handoff next planned work"
