...
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $OutputDir = Join-Path $RepoRoot "analysis-output\pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit"
}
...
if ($PlanOnly) {
    Write-Host "PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit plan OK."
...
$f64RefreshReport = Read-RepoJson -RelativePath "analysis-output\pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh\report.json"
$f64RefreshMarkdown = Read-RepoText -RelativePath "analysis-output\pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh\report.md"
...
Assert-All -Text $commandCaptureScript -Label "command capture F64/F65 source contract" -Needles @(
...
Assert-All -Text $commandCaptureScript -Label "command capture F64 closure source" -Needles $closureMarkers
...
Assert-All -Text $f64RefreshMarkdown -Label "F64 refresh gate markdown" -Needles @(
    'Completed task: `F64 refresh PC controlled-demo investor route evidence after F62 audit fixes`',
    'Next formal task: `F65 audit post-F64 PC controlled-demo investor route evidence refresh`',
    'NoUnityLaunch: True',
...
if ($null -ne $f64RefreshReport) {
    Assert-Equals -Actual ([string]$f64RefreshReport.schema) -Expected "PCControlledDemoInvestorRouteEvidenceAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefresh" -Label "F64 refresh report schema"
    Assert-Equals -Actual ([string]$f64RefreshReport.result) -Expected "pass" -Label "F64 refresh report result"
    Assert-Equals -Actual ([string]$f64RefreshReport.completedTask) -Expected "F64 refresh PC controlled-demo investor route evidence after F62 audit fixes" -Label "F64 refresh completed task"
    Assert-Equals -Actual ([string]$f64RefreshReport.nextFormalTask) -Expected "F65 audit post-F64 PC controlled-demo investor route evidence refresh" -Label "F64 refresh next task"
    Assert-Equals -Actual ([bool]$f64RefreshReport.noUnityLaunch) -Expected $true -Label "F64 refresh no Unity launch"
    Assert-Equals -Actual ([string]$f64RefreshReport.sourceAuditReport) -Expected "analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit/report.json" -Label "F64 source F62 audit"
    Assert-Equals -Actual ([string]$f64RefreshReport.sourceFixesReport) -Expected "analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fixes/report.json" -Label "F64 source F63 fixes"
}
...
Assert-All -Text $masterPlan -Label "master F65/F66 queue" -Needles @(
    'PC1-PC65',
    '| 143 | Done | `Audit post-F64 PC controlled-demo investor route evidence refresh` |',
    '| 144 | Next | `Implement post-F65 PC controlled-demo investor route evidence refresh audit fixes` |',
    'Mobile phones remain landscape-only for the first playable target'
)
Assert-All -Text $detailedPlan -Label "detailed F65/F66 queue" -Needles @(
    '| F65 | Done | `Audit post-F64 PC controlled-demo investor route evidence refresh` |',
    '| F66 | Next | `Implement post-F65 PC controlled-demo investor route evidence refresh audit fixes` |'
)
Assert-All -Text $mobilePlan -Label "mobile F65/F66 status" -Needles @(
    'PC1-PC65',
    'F65 audit post-F64 PC controlled-demo investor route evidence refresh',
...
Assert-All -Text $handoffDoc -Label "handoff F66 next" -Needles @(
...
Assert-All -Text $routeDoc -Label "route doc F65 audit visibility" -Needles @(
    ...
    'RouteAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFinding=route-proof-clarity status=pass',
...
Assert-All -Text $evidenceDoc -Label "evidence doc F65 audit visibility" -Needles @(
    'RouteAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAudit=pass-with-followups source=analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit/report.json completed=F65 next=F66 noUnityLaunch=True mobile=landscape-only',
    'RouteAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFinding=damage-ejection-proof status=pass',
...
Assert-All -Text $queueScript -Label "queue F65/F66 markers" -Needles @(
    'PC1-PC65',
    'F65 audit post-F64 PC controlled-demo investor route evidence refresh',
    'F66 implement post-F65 PC controlled-demo investor route evidence refresh audit fixes',
    '| F65 | Done | `Audit post-F64 PC controlled-demo investor route evidence refresh` |',
...
Assert-All -Text $currentGate -Label "current gate F65 audit plan" -Needles @(
    'audit_pc_controlled_demo_investor_route_evidence_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit.ps1',
    'PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit plan OK.'
)
Assert-All -Text $handoffScript -Label "handoff script F65/F66 markers" -Needles @(
    'audit_pc_controlled_demo_investor_route_evidence_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit.ps1',
    'PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit OK',
    'F66 implement post-F65 PC controlled-demo investor route evidence refresh audit fixes'
)
Assert-Contains -Text $gitignore -Needle 'analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit/' -Label ".gitignore F65 output"

Add-Finding -Area "route-proof-clarity" -Status "pass" -Detail "F64 command evidence keeps the five-preset Windows investor route and links command report, screenshots, sidecars and logs"
Add-Finding -Area "damage-ejection-proof" -Status "pass" -Detail "damage-demo still carries section loss, cockpit ejection, wreck salvage and repair-cost proof after the F64 refresh"
Add-Finding -Area "mobile-landscape-proof" -Status "pass" -Detail "F64 command evidence and plan surfaces keep first phone version landscape-only as a horizontal phone build"
...
Add-Finding -Area "audit-fix-refresh-closure" -Status "pass" -Detail "F64 command report and markdown carry F62 audit and F63 fix reports as explicit sources"

Add-FollowUp -Priority "P1" -Area "audit-visibility" -Issue "F65 audit findings should be made visible in plan, evidence and handoff surfaces before the next refresh step." -NextFix "Implement F66 as a narrow doc/gate update that records F65 audit findings, keeps no-Unity-launch evidence, and preserves the landscape-only phone scope."
Add-FollowUp -Priority "P2" -Area "next-refresh-contract" -Issue "The next command-evidence refresh should consume the F65 audit report explicitly so the audit closure can be traced." -NextFix "When F66 is complete, require its ignored report before advancing command evidence metadata again."

if ($failures.Count -gt 0) {
    Write-Host "PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit failed."
    foreach ($failure in $failures) {
        Write-Host " - $failure"
    }

    throw "$($failures.Count) PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit check(s) failed."
}

New-Item -ItemType Directory -Force -Path $OutputDir | Out-Null
$report = [pscustomobject]@{
    schema = "PCControlledDemoInvestorRouteEvidenceAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAudit"
    result = "pass-with-followups"
    timestampUtc = (Get-Date).ToUniversalTime().ToString("o")
    completedTask = "F65 audit post-F64 PC controlled-demo investor route evidence refresh"
    nextFormalTask = "F66 implement post-F65 PC controlled-demo investor route evidence refresh audit fixes"
    sourceCommandEvidenceReport = "analysis-output/pc-controlled-demo-command-evidence/pc-controlled-demo-command-evidence.json"
    sourceF64RefreshReport = "analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh/report.json"
    sourceF62AuditReport = "analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit/report.json"
    sourceF63FixReport = "analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fixes/report.json"
    noUnityLaunch = $true
...
$markdownLines.Add("# PC Controlled Demo Investor Route Evidence Audit Fix Refresh Audit Fix Refresh Audit Fix Refresh Audit Fix Refresh Audit Fix Refresh Audit Fix Refresh Audit")
$markdownLines.Add("")
$markdownLines.Add("Result: pass-with-followups")
$markdownLines.Add("")
$markdownLines.Add('Completed task: `F65 audit post-F64 PC controlled-demo investor route evidence refresh`')
$markdownLines.Add('Next formal task: `F66 implement post-F65 PC controlled-demo investor route evidence refresh audit fixes`')
$markdownLines.Add("")
...
$markdownLines.Add('Source F64 refresh: `analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh/report.json`')
...
Write-Host "PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit OK."
