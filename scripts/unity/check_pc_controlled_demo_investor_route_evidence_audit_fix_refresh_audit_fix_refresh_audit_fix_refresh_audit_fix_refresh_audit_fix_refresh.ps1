...
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $OutputDir = Join-Path $RepoRoot "analysis-output\pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh"
}
...
if ($PlanOnly) {
    Write-Host "PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh plan OK."
...
Assert-All -Text $commandCapture -Label "command capture F61/F62 metadata" -Needles @(
...
Assert-All -Text $commandCapture -Label "command capture F61 closure source" -Needles $closureMarkers
...
Assert-All -Text $currentGate -Label "current gate F61 plan marker" -Needles @(
    ...
    "PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh plan OK."
)

Assert-All -Text $queueScript -Label "queue F61/F62 marker" -Needles @(
    "F61 refresh PC controlled-demo investor route evidence after F59 audit fixes",
    "F62 audit post-F61 PC controlled-demo investor route evidence refresh",
    '| F61 | Done | `Refresh PC controlled-demo investor route evidence after F59 audit fixes` |',
    '| F62 | Next | `Audit post-F61 PC controlled-demo investor route evidence refresh` |'
)

Assert-All -Text $handoffScript -Label "handoff script F61/F62 marker" -Needles @(
    "check_pc_controlled_demo_investor_route_evidence_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh_audit_fix_refresh.ps1",
...
Assert-Contains -Text $gitignore -Needle "analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh/" -Label ".gitignore F61 output"

if ($failures.Count -gt 0) {
    Write-Host "PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh check failed."
    foreach ($failure in $failures) {
        Write-Host " - $failure"
    }

    throw "$($failures.Count) PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh check(s) failed."
}

New-Item -ItemType Directory -Force -Path $OutputDir | Out-Null
$report = [pscustomobject]@{
    schema = "PCControlledDemoInvestorRouteEvidenceAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefreshAuditFixRefresh"
    result = "pass"
    timestampUtc = (Get-Date).ToUniversalTime().ToString("o")
    completedTask = "F61 refresh PC controlled-demo investor route evidence after F59 audit fixes"
    nextFormalTask = "F62 audit post-F61 PC controlled-demo investor route evidence refresh"
    noUnityLaunch = $true
    ...
    sourceFixesReport = "analysis-output/pc-controlled-demo-investor-route-evidence-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fix-refresh-audit-fixes/report.json"
    refreshedAreas = @(
        ...
        "F60 closure is present in command markdown",
        "F60 closure is present in command JSON",
...
$markdownLines.Add("# PC Controlled Demo Investor Route Evidence Audit Fix Refresh Audit Fix Refresh Audit Fix Refresh Audit Fix Refresh Audit Fix Refresh")
$markdownLines.Add("")
$markdownLines.Add("Result: pass")
$markdownLines.Add("")
$markdownLines.Add('Completed task: `F61 refresh PC controlled-demo investor route evidence after F59 audit fixes`')
$markdownLines.Add('Next formal task: `F62 audit post-F61 PC controlled-demo investor route evidence refresh`')
$markdownLines.Add("")
...
Write-Host "PC controlled-demo investor route evidence audit fix refresh audit fix refresh audit fix refresh audit fix refresh audit fix refresh check OK."
