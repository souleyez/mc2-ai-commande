...
- Current plan queue consistency can be checked without launching Unity through `check_current_plan_queue.ps1`, proving README, BUILD-WIN, master/detailed/PC/mobile/evidence/handoff docs and helper scripts agree on the latest PC/mobile/platform checkpoint and `F10 wire optional Unity inventory bootstrap smoke` as the formal next task.
