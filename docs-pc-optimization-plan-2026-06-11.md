...
- Current plan queue, current plan gate, handoff and mobile command preflight accept PC1-PC59.
...
- Current plan queue, current plan gate, handoff and mobile command preflight accept PC1-PC59, `Pass Android G3 device smoke` and `G4 Touch UI pass`.
...
F54 PC/mobile boundary: landscape-only phone scope remains fixed while the Windows investor route evidence is refreshed; portrait phone UI is not part of the first slice.
