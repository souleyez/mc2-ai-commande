...
void app_update(app_t *self) {
	...
	glBindTexture(GL_TEXTURE_2D, self->texture_y);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, self->texture_cb);
	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, self->texture_cr);
...
void app_update_texture(app_t *self, GLuint unit, GLuint texture, plm_plane_t *plane) {
	...
	//);
	glTexImage2D(
		GL_TEXTURE_2D, 0, GL_R8, plane->width, plane->height, 0,
       GL_RED, GL_UNSIGNED_BYTE, plane->data
	);

	//glTexSubImage2D(
	//	GL_TEXTURE_2D, 0, 0, 0, plane->width, plane->height,
    //    GL_RED, GL_UNSIGNED_BYTE, plane->data);
}
